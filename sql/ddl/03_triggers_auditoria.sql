...
CREATE TRIGGER trg_audit_medical_offices AFTER INSERT OR UPDATE OR DELETE ON Medical_Offices FOR EACH ROW EXECUTE FUNCTION fn_audit_log();

DROP TRIGGER IF EXISTS trg_audit_speciality_offices ON Speciality_Offices;
CREATE TRIGGER trg_audit_speciality_offices AFTER INSERT OR UPDATE OR DELETE ON Speciality_Offices FOR EACH ROW EXECUTE FUNCTION fn_audit_log();
